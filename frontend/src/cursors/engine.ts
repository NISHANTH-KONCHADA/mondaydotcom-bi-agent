...
export class FlyingCursors {
    ...
    for (const def of this.defs) {
      ...
      root.innerHTML = `
        ...
        <span style="
          ...
          padding:1.5px 5px; border-radius:6px; border-top-left-radius:2px;
  ...
  private layout() {
    ...
    this.nodes.forEach((node, i) => {
      node.cx = (0.2 + 0.6 * ((i + 0.5) / n)) * w;
      node.cy = (0.35 + 0.3 * (i % 2)) * h;
      node.rx = (0.25 + 0.1 * (i % 2)) * w;
      node.ry = (0.28 + 0.08 * ((i + 1) % 2)) * h;
      node.phase = (i / n) * TWO_PI;
      node.speed = 0.5 + 0.2 * i;
    });
  }
  ...
  private loop = () => {
    ...
    for (const node of this.nodes) {
      ...
      if (this.pointer.active) {
        ...
        const push = Math.max(0, 1 - dist / 180) * 90;
        bx += (dx / dist) * push;
        by += (dy / dist) * push;
      }
